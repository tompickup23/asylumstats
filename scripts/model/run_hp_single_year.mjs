 ...
 * - 2011 base: NEWETHPOP Population2011_LEEDS2.csv (12 groups, split to 20 using 2021 proportions)
 ...
 *
 * METHODOLOGY NOTE on 2011 base splitting:
 * NEWETHPOP 2011 uses 12 groups. Census 2021 uses 20. To compute CCRs at 20-group level,
 * we split each NEWETHPOP 2011 12-group cell proportionally using the Census 2021 sub-group
 * shares within each parent group. This assumes sub-group proportions within parent groups
 * were approximately similar in 2011 — a standard demographic approximation when expanding
 * categories across time. The CCRs then capture the differential between the (estimated) 2011
 * sub-group population and the (observed) 2021 sub-group population.
 */
...
// ============================================================
// Parse NEWETHPOP 2011 base (12 groups) and split to 20
// ============================================================
...
// Split NEWETHPOP 2011 12-group to 20-group using Census 2021 proportions
console.log("Splitting 2011 data from 12 to 20 groups using 2021 proportions...");
...
for (const code of Object.keys(base2021.areas)) {
  ...
  for (const sex of SEXES) {
    for (let age = 0; age <= 90; age++) {
      // For each NEWETHPOP parent group, compute 2021 sub-group proportions
      for (const [parentEth, children] of Object.entries(NEWETHPOP_TO_CHILDREN)) {
        ...
        if (parentPop2011 <= 0) {
          // If no 2011 pop, distribute zero to all children
          for (const child of children) {
            pop2011.set(`${code}|${child}|${sex}|${age}`, 0);
          }
          continue;
        }
        ...
        // Compute 2021 sub-group proportions within this parent
        let parentTotal2021 = 0;
        for (const child of children) {
          parentTotal2021 += base2021.areas[code]?.[child]?.[sex]?.[age] || 0;
        }

        if (parentTotal2021 <= 0) {
          // If no 2021 data, distribute evenly
          for (const child of children) {
            pop2011.set(`${code}|${child}|${sex}|${age}`, parentPop2011 / children.length);
          }
        } else {
          // Split proportionally using 2021 sub-group shares
          for (const child of children) {
            const childPop2021 = base2021.areas[code]?.[child]?.[sex]?.[age] || 0;
            const share = childPop2021 / parentTotal2021;
...
console.log(`  Split complete: ${pop2011.size} cells`);
